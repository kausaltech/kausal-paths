...
class DjangoPythonFilter(watchfiles.PythonFilter):
    def __init__(
        ...
        *,
        ...
        for path in ('e2e-tests', 'query-store', '__generated__', '.mypy_cache', '.ruff_cache'):
...
class WatchfilesReloader(autoreload.BaseReloader):
    def watched_roots(self, watched_files: list[Path]) -> frozenset[Path]:
        ...
        sys_paths = {path for path in autoreload.sys_path_directories() if 'site-packages' in str(path)}
