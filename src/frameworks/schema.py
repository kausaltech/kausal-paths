...
class MeasureType(DjangoNode[Measure]):
    ...
    def _get_placeholder_df(
        ...
        """
        # A measure reached only through a goal-tagged binding is a target-year input --
        # "Share of heating as district heating, the target year", "Renovation rate -
        # decarbonisation scenario" -- and this tab asks what happened in the years up to
        # today. It has no historical series to show, and its uuid landed on a goal column
        # because that is the only column carrying it, not because the target is the
        # answer. The node it sits on is an action, whose output is movement from the
        # baseline year, so the number available here is neither the target nor a level:
        # a grey `-3.7` in a cell asking for a share reads as authoritative and means the
        # planned reduction.
        #
        # So say nothing. On budapest-cap-2030 this is 21 of the 159 mapped measures, all
        # of them target-year assumptions, and all of them blank before the binding path
        # existed. If cities should see their own 2030 assumption echoed here, the value to
        # show is the goal itself -- constant across the window -- not this node's output.
        if selection.binding_role == 'goal':
            logger.info(
                f'Measure {measure.measure_template.uuid} is only carried by a goal-tagged binding '
                f'on node {node.id}; it has no historical series, so no placeholders'
            )
            return None
    ...
    def resolve_placeholder_data_points(root: Measure, info: GQLInfo) -> list[PlaceHolderDataPoint]:
        ...
        return MeasureType._narrow_to_placeholders(
            ...
            baseline_year=fwc.reference_year,
