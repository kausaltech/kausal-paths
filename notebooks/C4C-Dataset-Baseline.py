...
unitreplace = [['tCO2e', 't'], ['vkm', 'km'], ['p-km', 'km'], ['Mkm', 'Gm'], ['€', 'EUR']]
...
def unitcheck(df, vlist, rfile):
    ...
    if units.count(None) == 1:
        ...
        for y in vlist:
            df = df.with_columns(
                pl.struct(pl.col(y).alias('Value'), pl.col('Unit'), pl.lit(units[0]).alias('ToUnit'))
                .map_elements(pconvert)
                .alias(y)
            )
...
for sector in sectors:
    ...
    for qtype in ['Emissions', 'Activity', 'Price']:
        dff = df.filter((pl.col('Sector') == sector) & (pl.col('Quantity Type') == qtype))
...
dfmain = (
    df.head(1)
    .select(context)
    .with_columns([(pl.lit(0.0).alias('Value').cast(pl.Float64)), (pl.lit(0).alias('Year').cast(pl.Int64))])
    .clear()
)
