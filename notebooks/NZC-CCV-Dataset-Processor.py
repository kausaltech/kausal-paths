...
incin_df = (
    ...
    .group_by('Cluster_42')
    .agg(pl.all().mean())
    .select(incin_measures + ['Cluster_42'])
)
...
all_df = all_df.with_columns(pl.when(pl.col('Measure').is_in(pop_measures)).then(True).otherwise(False).alias('PerCapita'))  # noqa: FBT003
...
for cluster in ['0', '1', '2', '3']:
    all_df = all_df.with_columns(
        pl.when(pl.col('Unit').is_not_null()).then(pl.col(cluster) * 100).otherwise(pl.col(cluster)).alias(cluster)
    )
