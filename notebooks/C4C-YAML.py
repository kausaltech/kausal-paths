...
levellookup = {
    ...
    1: {'I': 'Stationary', 'II': 'Transportation', 'III': 'Waste', 'IV': 'IPPU', 'V': 'AFOLU'},
}

quantitylookup = {
    'Emission Factor': {'name': 'Emission Factor', 'quantity': 'emission_factor'},
...
enodes = {makeid(terminal['e']): {'template': 'emissions', 'name': terminal['e'], 'upstream': []}}
pnodes = {
    makeid(terminal['p']): {
        'template': 'simple',
        'name': terminal['p'],
        ...
        if not gpcdf.is_empty():
            ...
            if (len(activities) == 1) & ('Emission Factor' in list(frames.keys())):
                ...
                leafnodes[enodeid] = {
                    ...
                    'quantity': quantitylookup['Emissions']['quantity'],
                    'unit': '%s/%s'
                    % (leafnodes[upstream[1]]['unit'].split('/')[0], leafnodes[upstream[0]]['unit'].split('/')[1]),
                    'dimlist': list(set(leafnodes[upstream[0]]['dimlist']) | set(leafnodes[upstream[1]]['dimlist'])),
                    'upstream': upstream,
                    'downstream': [edown],
                }
