...
class NodePayload:
    ok: bool
    node: Annotated[NodeInterface, sb.lazy('nodes.schema')] | None = None
...
class InstanceEditorMutation:
    ...
    def update_node(info: gql.Info, root: sb.Parent[Me], node_id: sb.ID, input: UpdateNodeInput) -> 'Node':
        ...
        nc.spec = spec
        updates['spec'] = spec
        # Build the set of DB columns to update alongside the spec.
        # We use queryset.update() to avoid ClusterableModel.save()
        # clobbering modeltrans fields.
