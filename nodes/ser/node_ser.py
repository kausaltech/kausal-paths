...
from typing import TYPE_CHECKING, Annotated, ClassVar, Literal, Optional, TypeAlias
...
EdgeTransformationType: TypeAlias = Literal['select_categories', 'assign_category']


class EdgeTransformation(BaseModel, abc.ABC):
    # TYPE: ClassVar[EdgeTransformationType]

    # type: EdgeTransformationType
    label: Optional[str]
    "Label for the processing step"


class SelectCategoriesEdgeTransformation(EdgeTransformation):
    # TYPE: ClassVar = 'select_categories'
...
class EdgeSer(IdentifiedModel):
    ...
    @staticmethod
    def _make_input_ports(edge: Edge, nodes: dict[str, NodeSer]):
    ...
    @classmethod
    def from_edge(cls, edge: Edge, nodes: dict[str, NodeSer]):
        ...
        return cls(source=src, sink=sink)
...
class PathsModelConfig(BaseModel):
    ...
    def from_instance(cls, instance: Instance) -> PathsModelConfig:
        ...
        n = ctx.nodes['transportation_emissions']
        included_nodes = set(n.get_upstream_nodes())
