"""
Import a YAML instance configuration into the Trailhead DB models.
...
class Command(BaseCommand):
    help = 'Import a YAML instance config into Trailhead DB models'
...
def _import_instance(data: dict[str, Any], stdout: Any, stderr: Any) -> Any:
...
def _update_instance_fields(ic: Any, data: dict[str, Any]) -> None:
...
def _clear_trailhead_data(ic: Any) -> None:
    from nodes.models import ActionGroup, DatasetPort, NodeEdge, Scenario

    NodeEdge.objects.filter(instance=ic).delete()
    DatasetPort.objects.filter(instance=ic).delete()
    ActionGroup.objects.filter(instance=ic).delete()
    Scenario.objects.filter(instance=ic).delete()


def _import_action_groups(ic: Any, data: dict[str, Any]) -> dict[str, Any]:
...
def _import_scenarios(ic: Any, data: dict[str, Any]) -> None:
...
def _node_defaults(node_data: dict[str, Any], is_action: bool, ag_map: dict[str, Any]) -> dict[str, Any]:
    node_modeled_keys = {
        'id',
        'identifier',
        'name',
