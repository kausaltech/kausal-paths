...
class ObservableNode(GenericNode):
    """
    Switches between pure model output and observed city data based on use_observations.

    - use_observations=False (default scenario): returns the ``modeled``-tagged
      input node's output unchanged.  The model already incorporates city
      reference data through its upstream DVC inputs, so no dataset anchoring
      is needed here.
    - use_observations=True (progress-tracking): returns the input dataset
      processed by FrameworkMeasureDVCDataset2, which overlays DB
      MeasureDataPoints for framework instances.
    """
    ...
    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)
        self.OPERATIONS['get_observable'] = self._operation_get_observable

    def _get_add_multiply_nodes(self) -> tuple[list[Node], list[Node]]:
        return [], []  # all input handling done in get_observable

    def _operation_get_observable(self, df: PathsDataFrame | None) -> PathsDataFrame:
        ...
        if not use_obs:
            if modeled_node is None:
                raise NodeError(self, "ObservableNode requires a 'modeled' input node")
            return modeled_node.get_output_pl(target_node=self)

        # Progress-tracking: use the dataset with DB measure-data-point overrides.
        dataset_df = self.get_input_dataset_pl(required=False)
        if dataset_df is not None:
            drop_cols = ['ObservedDataPoint', 'FromMeasureDataPoint', 'observed', 'placeholder']
        ...
        if modeled_node is not None:
            return modeled_node.get_output_pl(target_node=self)
        raise NodeError(self, "ObservableNode has no dataset and no 'modeled' input node")
