...
def _generate_forecast_rows(df: pd.DataFrame, target_year: int) -> (pd.DataFrame, int):
...
def extend_last_historical_value(df: pd.DataFrame, target_year: int) -> pd.DataFrame:
    ...
    df.loc[row_selector, VALUE_COLUMN] = df.loc[row_selector, VALUE_COLUMN].astype('float64').fillna(method='pad').astype(dt)
    return df
