...
class DataFrameMeta:
    ...
    def copy(self) -> DataFrameMeta:
        return DataFrameMeta(units=self.units.copy(), primary_keys=self.primary_keys.copy(), explanation=self.explanation.copy())
...
class PathsDataFrame(pl.DataFrame):
    ...
    def get_meta(self) -> DataFrameMeta:
        ...
        meta = DataFrameMeta(
            units=self._units.copy(), primary_keys=self._primary_keys.copy(), explanation=explanation_list.copy()
        )
        return meta
...
def to_ppdf(df: pl.DataFrame | PathsDataFrame, meta: DataFrameMeta | None = None) -> PathsDataFrame:
    ...
    if meta is None:
        meta = DataFrameMeta(units={}, primary_keys=[], explanation=source_explanation)
